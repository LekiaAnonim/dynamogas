...
class DynamoGas(toga.App):
    ...
    def startup(self):
        ...
        """
        label_font_size = 10
        label_font_color = '#231955'  # '#ffe5b4'
        ...
        self.price_perkg_input = toga.NumberInput(
            style=Pack(flex=1, padding_left=5))
        ...
        self.amount_paid_input = toga.NumberInput(
            style=Pack(flex=1, padding_left=5))
        ...
        self.number_kg_input = toga.NumberInput(
            style=Pack(flex=1, padding_left=5))
        ...
        self.purchase_price_input = toga.NumberInput(
            style=Pack(flex=1, padding_left=5))
        ...
        split = toga.Box(style=Pack(direction=COLUMN, padding=0))
        split.add(group1_box, group2_box)
        # Calcuated
        # Create balance box
        ...
        kg_calc_button = toga.Button('Calculate Kg', on_press=self.NumberOfKg, style=Pack(width=105,
                                                                                          text_align=LEFT, padding_left=0, background_color=button_1_color, color='#fff', font_size=button_1_font_size, font_family=font_family))
        ...
        price_calc_button = toga.Button('Calculate price', on_press=self.PurchasePrice, style=Pack(width=105,
                                                                                                   text_align=LEFT, padding_left=0, background_color=button_1_color, color='#fff', font_size=button_1_font_size, font_family=font_family))
        ...
        self.cleared = toga.Selection(
            items=['Cleared', 'Not Cleared'], style=Pack(flex=1, padding_left=0, background_color='#fff'))
        ...
        print_button = toga.Button('Print', on_press=self.print_slip, style=Pack(
        ...
        transaction_entry_box.add(split)
        ...
        self.transaction_entry_container = toga.ScrollContainer(
            horizontal=False)
        self.transaction_entry_container.content = transaction_entry_box
        ...
        self.date_input = toga.TextInput(
            placeholder='Enter date here', style=Pack(flex=1, height=50, font_size=13))
        ok_button = toga.Button('OK', on_press=self.Analysis, style=Pack(
            padding_top=2, width=40, background_color='#001d6e', color='#fff'))
        ...
        self.searchby_opt = toga.Selection(
            items=['Name of Customer', 'Date', 'Phone Number', 'Status'], style=Pack(flex=1))
        ...
        delete_search_table_button = toga.Button('delete tables',
            on_press=self.delete_search_analysis_table, style=Pack(font_family='monospace', width=130, padding_left=20, font_size=10, color='red', background_color='#fff'))
        ...
        self.tran_cont_box.add(date_inst_box, searchby_box,
                               search_box, delete_search_table_button, self.trans_analyis_box)
        ...
        entry_command = toga.Command(self.EntryWindow, tooltip="Enter customer's transaction details",
                                     label='Transaction Entry', icon=add_user_icon, group=tools)

        analysis_command = toga.Command(self.AnalysisWindow, tooltip="Analyse data by date ",
                                        label='Transaction Analysis', icon=analysis_icon, group=tools)

        entry_button = toga.Button('Enter transaction details',
                                   on_press=self.EntryWindow, style=Pack(padding_left=50, padding_right=50, padding_bottom=30, height=50, background_color='#4b7be5', color='#fff', font_family=font_family, font_size=16))
        ...
        self.main_box = toga.Box(style=Pack(
            direction=COLUMN, background_color='#c4ddff'))
        self.main_box.add(entry_button, self.tran_cont_box)

        self.main_box_container = toga.ScrollContainer(
            horizontal=False)

        self.main_box_container.content = self.main_box
    ...
    def NumberOfKg(self, widget, **kwargs):
        Kg = "%8.2f" % (
            float(self.purchase_price_input.value)/float(self.price_perkg_input.value))
        self.calc_kg_calc.value = str(Kg)

        return self.calc_kg_calc

    def PurchasePrice(self, widget, **kwargs):
        Price = "%8.2f" % (float(self.price_perkg_input.value)
                           * float(self.number_kg_input.value))
        self.calc_price_calc.value = str(Price)
        return self.calc_price_calc

    def Balance(self, widget, **kwargs):
        balances = "%8.2f" % (
            float(self.amount_paid_input.value)-float(self.purchase_price_input.value))
    ...
    def EntryWindow(self, widget, **kwargs):
        self.entry_box = toga.Box(style=Pack(
            direction=COLUMN, background_color='#c4ddff'))  # e8aa42
